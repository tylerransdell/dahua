...
class DahuaClient:
    ...
    async def async_set_lighting_v2(self, channel: int, enabled: bool, brightness: int, profile_mode: str, light_types: list = None, use_zoomprio: bool = False) -> dict:
        """
        ...
        use_zoomprio: if True, use ZoomPrio mode (ON=ZoomPrio+Correction=50, OFF=Off)
        """

        if use_zoomprio:
            # ZoomPrio cameras: Mode=ZoomPrio turns on the light via zoom-priority algorithm
            # Correction=50 is the recommended value for ZoomPrio
            mode = "ZoomPrio"
            if not enabled:
                mode = "Off"
            url = "/cgi-bin/configManager.cgi?action=setConfig&Lighting_V2[{channel}][{profile_mode}][0].Mode={mode}&Lighting_V2[{channel}][{profile_mode}][0].Correction=50".format(
                channel=channel, profile_mode=profile_mode, mode=mode
            )
            _LOGGER.debug("Setting ZoomPrio light: %s", url)
            return await self.get(url)
        ...
        # For MiddleLight and other non-ZoomPrio light types, always set brightness to 100 (full)
        # when enabled, since the normalization in hass_brightness_to_dahua_brightness can produce
        # unexpected values (e.g. 39%) from the HA brightness scale (0-255).
        if enabled:
            brightness = 100
